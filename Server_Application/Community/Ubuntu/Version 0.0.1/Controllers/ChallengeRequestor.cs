...
namespace PKDSA_ServerApp.Controllers
{
    ...
    public class ChallengeRequestor : ControllerBase
    {
        ...
        [HttpGet]
        public LoginModels RequestChallenge(String User_ID,String Key_Identifier)
        {
            ...
            MySQLGeneralQuery = new MySqlCommand();
            MySQLGeneralQuery.CommandText = "INSERT INTO `Random_Challenge`(`Challenge`, `Valid_Duration`, `User_ID`, `Key_Identifier`) VALUES (@Challenge,@Valid_Duration,@User_ID,@Key_Identifier)";
            MySQLGeneralQuery.Parameters.Add("@Challenge", MySqlDbType.Text).Value = Convert.ToBase64String(RandomData);
        ...
        public LoginModels GetChallenge(String User_ID, String Key_Identifier)
        {
            LoginModels MyLoginModels = new LoginModels();
            VerifyDataClass verifyDataClass = new VerifyDataClass();
            DecodeDataClass decodeDataClass = new DecodeDataClass();
            ...
            myMyOwnMySQLConnection.LoadConnection(ref ExceptionString);
            MySQLGeneralQuery = new MySqlCommand();
            MySQLGeneralQuery.CommandText = "SELECT COUNT(*) FROM `Random_Challenge` WHERE `User_ID`=@User_ID AND `Key_Identifier`=@Key_Identifier";
            MySQLGeneralQuery.Parameters.Add("@User_ID", MySqlDbType.Text).Value = User_ID;
            MySQLGeneralQuery.Parameters.Add("@Key_Identifier", MySqlDbType.Text).Value = Key_Identifier;
            MySQLGeneralQuery.Connection = myMyOwnMySQLConnection.MyMySQLConnection;
            MySQLGeneralQuery.Prepare();
            ...
            else
            {
                ...
                {
                    MySQLGeneralQuery = new MySqlCommand();
                    MySQLGeneralQuery.CommandText = "SELECT `Challenge` FROM `Random_Challenge` WHERE `User_ID`=@User_ID AND `Key_Identifier`=@Key_Identifier";
                    MySQLGeneralQuery.Parameters.Add("@User_ID", MySqlDbType.Text).Value = User_ID;
                    MySQLGeneralQuery.Parameters.Add("@Key_Identifier", MySqlDbType.Text).Value = Key_Identifier;
                    MySQLGeneralQuery.Connection = myMyOwnMySQLConnection.MyMySQLConnection;
                    MySQLGeneralQuery.Prepare();
