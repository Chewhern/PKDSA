...
namespace PKDSA_ServerApp.Controllers
{
    ...
    public class RegisterAccount : ControllerBase
    {
        ...
        [HttpGet]
        public String CreateAccount(String User_ID,String Session_ID,String URLEncoded_Master_Signed_PK,String URLEncoded_Master_PK)
        {
            String Status = "";
            DecodeDataClass decodeDataClass = new DecodeDataClass();
            String DecodedMaster_Signed_PK = "";
            Boolean DecodingMaster_Signed_PK = true;
            Boolean HexaBoolean = true;
            ...
            {
                HexaBoolean = HexaChecker.CheckHexa(Session_ID);
                if (HexaBoolean == true)
                {
                    ...
                    if (UserIDChecker == 0)
                    {
                        ...
                        MyMyOwnMySQLConnectionClass.LoadConnection(ref ExceptionString);
                        MySQLGeneralQuery.CommandText = "INSERT INTO `User`(`User_ID`, `Session_ID` ,`Master_Signed_PK`, `Master_PK`) VALUES (@User_ID,@Session_ID,@Master_Signed_PK,@Master_PK)";
                        MySQLGeneralQuery.Parameters.Add("@User_ID", MySqlDbType.Text).Value = User_ID;
                        MySQLGeneralQuery.Parameters.Add("@Session_ID", MySqlDbType.Text).Value = Session_ID;
                        ...
                        Status = "Congratulations: Your account has been registered ...";
                    }
                    else
                    {
                ...
                {
                    Status = "Error: Session's Messenger's user ID must be hexadecimal and don't use any other data";
                }
            }
            else
            {
