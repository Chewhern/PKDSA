...
namespace PKDSA_UMWinClient.APIMethodHelper
{
    ...
    {
        public static void RequestMasterChallengeFromServer(String User_ID,ref LoginModels myModel) 
        {
            if (TORMainOperations.UsingTor == true)
            {
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    var response = client.GetAsync("MasterChallengeRequestor?User_ID=" + User_ID);
                    response.Wait();
        ...
        public static void GetLostChallengeFromServer(String User_ID, ref LoginModels myModel)
        {
            if (TORMainOperations.UsingTor == true)
            {
                var handler = new HttpClientHandler
                ...
                using (var client = new HttpClient(handler))
                {
                    ...
                    client.DefaultRequestHeaders.Accept.Add(
                        new MediaTypeWithQualityHeaderValue("application/json"));
                    var response = client.GetAsync("MasterChallengeRequestor/GetLostChallenge?User_ID=" + User_ID);
                    response.Wait();
                    var result = response.Result;
                    if (result.IsSuccessStatusCode)
                    {
