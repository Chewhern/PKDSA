...
namespace PKDSA_UMWinClient
{
    public partial class LoginUsingSubKeys : Form
    {
        ...
        private async void RequestChallengeBTN_Click(object sender, EventArgs e)
        {
            if (SubKeyIdentifierTB.Text.CompareTo("") != 0)
            {
                ...
                try
                {
                    myLoginModel = await ChallengeRequestor.RequestChallengeFromServer(User_ID, Key_Identifier);
                }
                catch 
                {
                    myLoginModel = await ChallengeRequestor.GetLostChallengeFromServer(User_ID, Key_Identifier);
                }                
        ...
        private async void LoginBTN_Click(object sender, EventArgs e)
        {
            if (SubKeyIdentifierTB.Text.CompareTo("") != 0 && UserSignedChallengeTB.Text.CompareTo("")!=0)
            {
                ...
                {
                    StatusTB.Text = await TORProxyLoginUsingSubKeys(URL);
                }
                else
                {
        ...
        private async Task<String> TORProxyLoginUsingSubKeys(String URL) 
        {
