...
namespace PKDSA_UMWinClient
{
    public partial class ChangeMasterKey : Form
    {
        ...
        private async void SubmitInfoBTN_Click(object sender, EventArgs e)
        {
            ...
            {
                ChangeKeysStatusTB.Text = await TORProxyChangeMasterKey(User_ID,UserSignedChallenge);
            }
            else 
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (result.IsSuccessStatusCode)
                    {
                        ...
                        ChangeKeysStatusTB.Text = Result.Substring(1, Result.Length - 2);
        ...
        private async Task<String> TORProxyChangeMasterKey(String User_ID,Byte[] UserSignedChallenge) 
        {
            ...
            using (var client = new HttpClient(handler))
            {
                ...
                client.DefaultRequestHeaders.Accept.Add(
                    new MediaTypeWithQualityHeaderValue("application/json"));
                var response = await client.GetAsync("ChangeKey?User_ID=" + User_ID + "&URLEncoded_Signed_Challenge=" + System.Web.HttpUtility.UrlEncode(Convert.ToBase64String(UserSignedChallenge)) + "&URLEncoded_Signed_PK=" + System.Web.HttpUtility.UrlEncode(SignedPublicKeyTB.Text) + "&URLEncoded_PK=" + System.Web.HttpUtility.UrlEncode(PublicKeyTB.Text));
